...
void Search_by_key(char *file_directory, char* filename)
{
    ...
    while(fread(&record, sizeof(contact), 1, fp)) {
        if(strcmp(search_number, record.phone_number) == 0) {
            found = 1;
            printf("\nRecord found:\n");
...
void Delete_by_key(char *file_directory, char* filename)
{
    ...
    fp = fopen(Concatination(file_directory, ".txt"), "wb");
    ...
    for (int j = 0; j < i; j++) {
        fwrite(&records[j], sizeof(contact), 1, fp);
    }
    ...
    printf("\nContact with phone number %s deleted successfully\n", search_number);
}
